...
from app.services import application_svc, export_svc
...
@router.get("/v1/applications/{application_id}/export")
async def export_application(
    ...
    """Download the application as PDF / DOCX / CSV, in any state.

    For an *approved* application a PDF request returns the signed PDF
    (letterhead + approval chain + verification QR) generated by application_svc.
    """
    app = await application_svc.get_application(db, application_id, token.user_id, token.tenant_id)
    if app is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Application not found")
    ...
    if format.lower() == "pdf" and app.state == ApplicationState.approved:
        return Response(
            content=application_svc._build_pdf(app),
            media_type="application/pdf",
            headers={"Content-Disposition": f'attachment; filename="DIU-APP-{short_id}.pdf"'},
        )

    template_name = app.template.name if app.template else "Application"
    date_str = (app.approved_at or app.created_at).strftime("%d %b %Y")
    meta = [
        ("Reference", f"DIU-APP-{short_id}"),
        ("Status", str(app.state).replace("_", " ").title()),
        ("Date", date_str),
    ]
    doc = export_svc.ExportDoc(
        title=template_name,
        subtitle="Campus Application",
        meta=meta,
        body=app.letter_body or "",
    )
    return export_svc.export_response(doc, format, f"DIU-APP-{short_id}")
